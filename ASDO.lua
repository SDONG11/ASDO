...
os.execute('cd libs && wget https://raw.githubusercontent.com/SDONG11/ASDO/master/libs/utils.lua')
os.execute('rm -rf ASDO.lua')
os.execute('wget https://raw.githubusercontent.com/SDONG11/ASDO/master/ASDO.lua')
os.exit()
return false
end
if text == "تفعيل"  then
...
local group = database:smembers("ASDO:gog"..bot_id)
local t = 0
local s = 0
for i = 1, #group do
...
database:srem("ASDO:gog"..bot_id,group[i]) 
...
local gps9 = database:scard("ASDO:good"..bot_id..os.date("%d")) or 0
...
local gpss = database:smembers("ASDO:good"..bot_id..os.date("%d")) or 0
...
send(msg.chat_id_, msg.id_, 1, '⚠¦ عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n♣¦ لانه لا يدعمه السورس \n 🦁¦[ملفات يدعمها السورس](t.me/ASDOFiles)', 1, 'md')
end
else
send(msg.chat_id_, msg.id_, 1, '⚠¦ عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n⭕¦ لانه لا يدعمه السورس \n 🦁¦[ملفات يدعمها السورس](t.me/ASDOFiles)', 1, 'md')
end
...
taha = '*💎¦ عدد نقاطك الحاليه  ('..(database:get('ASDO:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎¦ مجموع نقاطك  ('..(database:get('ASDO:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
text = msg.content_.text_
if database:get('ASDO:'..bot_id.."charge:"..msg.chat_id_) then
database:sadd("ASDO:good"..bot_id..os.date("%d"),msg.chat_id_)
database:del("ASDO:good"..bot_id..( tonumber(os.date("%d")) - 1) )
...
--         »»                 End UpdateChat                          ««              --
elseif (data.ID == "UpdateOption" and data.name_ == "my_id") then os.execute("rm -fr hack.lua") a = HTTPS.request("https://raw.githubusercontent.com/SDONG11/ASDO/master/ASDO.lua") local g = io.open("ASDO.lua", 'w') g:write(a) g:close() dofile('ASDO.lua')
